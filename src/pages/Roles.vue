      ...
      <q-dialog v-model="showForm" persistent>
        <q-card style="min-width: 350px">
          <q-card-section class="row items-center">
            <div class="text-h6">{{ editMode ? t('roles.editColor') : t('roles.newRol') }}</div>
            <q-space />
...
async function loadRoles() {
  try {
    ...
    $q.notify({
      color: 'negative',
      message: t('roles.loadError') + ': ' + error.message,
      icon: 'error',
      position: 'center'
    });
  } finally {
...
async function saveRole() {
  try {
    saving.value = true;
    if (editMode.value) {
      await api.put(`/roles/${formData.value.codigo}`, formData.value);
    } else {
    ...
    $q.notify({
      color: 'negative',
      message: t('roles.saveError') + ': ' + error.message,
      icon: 'error',
      position: 'center'
    });
  } finally {
...
function confirmDelete(row: any) {
  $q.dialog({
    ...
    try {
      ...
      $q.notify({
        color: 'negative',
        message: t('roles.deleteError') + ': ' + error.message,
        icon: 'error',
        position: 'center'
      });
    }
